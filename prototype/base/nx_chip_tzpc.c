...
void NX_DREX_SetTZASC(U32 index, U32 addr, U32 size, U32 type)
{
	...
	if (type == TZASC_SECURE)
		secure_type = 0xC; //WriteIODW(reg, (0xC << 28 | size<<4 | 1<<0)); // secure access | size | enable
	else if (type == TZASC_NONSECURE)
		secure_type = 0xF; //WriteIODW(reg, (0xF << 28 | size<<4 | 1<<0)); // secure access | size | enable
	else if (type == TZASC_NONSECURE_ONLY)
		secure_type = 0x3; //WriteIODW(reg, (0x3 << 28 | size<<4 | 1<<0) ); // secure access | size | enable
	else
		secure_type = 0xF; //WriteIODW(reg, (0xF << 28 | size<<4 | 1<<0) ); // secure access | size | enable
